...
function merge(parentObj, props, value) {
    ...
    if (props.length > 0 && defaultMediaKeys[currentNodeName]) {
        ...
        console.log('oooo', props, currentNodeName);
    }
    ...
    if (props.length > 1) {
        var parentNodeName = props[props.length - 2];
        if (defaultMediaKeys[parentNodeName]) {
            var parentNode2 = _buildChildren(props.slice(0, -2), parentObj);
...
/*
    // Case for og:video:type before og:video:url.
    var prevProp = props[props.length - 2];
    if (props.length > 1 && defaultMediaKeys[prevProp]) {
        parentNode2 = _buildChildren(props.slice(0, -2), parentObj);
        if (currentNodeName in parentNode2) {
            var _value = {};
            _value[currentNodeName] = value;
            value = _value;
            parentNode = parentNode2;
            currentNodeName = prevProp;
            //console.log('catch', props);
        }
    }
*/

    console.log('props', props, currentNodeName);
    console.log('currentNodeName', currentNodeName);
    console.log('parentNode', JSON.stringify(parentNode, null, 4));
    console.log('-------------');
    ...
    if (!(currentNodeName in parentNode)) {
        ...
        //console.log('-', currentNodeName, props, value);
...
/*
            var parentNodeName = props[props.length - 2];
            if (defaultMediaKeys[parentNodeName]) {
                var parentNode = _buildChildren(props.slice(0, -2), parentObj);
                parentNode
            }
*/
            // Create array.
